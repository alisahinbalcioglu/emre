...
### DWG — Kiracı Dedup Kapsamı ve file_id Kapısı (26.09.2026)
- Motor `/upload` aynı içeriği (sha256) YALNIZ aynı `kapsam`da tekilleştirir. Kapsam = Nest'in `dedupKapsami(firmaId)` (`dwg-sahiplik.servisi.ts`): firmaya özgü, firma içinde sabit, anahtarsız sha256 — firma kimliği motora HAM gitmez ama değer gizli DEĞİLDİR (amaç ayırmak; kimlik kanıtı olarak kullanılmaz, HMAC gereksiz — güvenlik incelemesi). Kapsamsız (ya da boş) yükleme HİÇ tekilleştirilmez. Eskiden tekilleştirme TÜM kiracılar arasındaydı: ikinci firma birincinin file_id'sini `dedup: true` ile alıyor, sahiplik ilk firmada kaldığı için kendi yüklemesinin /status, /geometry, /parse çağrılarında 403 yiyor ve aynı çizimi başkasının yüklediğini öğreniyordu (ön yüz "Bu dosya daha önce yüklenmişti" diyordu). Gerçekçi senaryo: aynı ihale çizimi birden çok yükleniciye gider. Gerçek motor + gerçek Nest ile yeniden üretildi; canlıda 26.09'a kadar DWG yükleyen tek firma vardı (8/8 DwgDosya satırı), yani yaşanmadı.
- KARAR (Emre 26.09): firma başına kapsam. Reddedilen: başka firmanın isabetinde kopya/hardlink ile yeni file_id — sahibi motor bilmeli, işlenirken isabette yarış, hardlink ilk yüklemenin TTL'ini taşır (erken 410), sızıntı tam kapanmaz (ikinci firmaya anında "ready" = "başkası yüklemiş"). Bedel: aynı dosyayı yükleyen ikinci firma için dönüşüm+ayrıştırma yeniden koşar.
- SÜRÜM KAYMASI (eski motor kapsamı bilmez): motor kapsamı uyguladığında `/upload` yanıtı `kapsamli: true` taşır; Nest bu işareti görmezse HER yüklemeyi aynı 503 ile reddeder (ilk firma dahil). Yalnız tekilleştirilen dosya reddedilseydi, bir baytı değişmiş kopya geçtiği için "bu çizimi başkası yüklemiş" yine okunurdu (güvenlik incelemesi 26.09). Bedel: kayma sürerken DWG yükleme tamamen durur — gürültülü ve güvenli; deploy.sh motor `build_sha`'sını ayrıca denetler.
- Savunma katmanı: `kaydet` başka firmaya ait kimliği ASLA geri vermez — motor "kapsamladım" deyip yine de tekilleştirirse "DWG CAPRAZ-FIRMA DEDUP" ERROR + aynı genel 503. İstemciye yalnız `file_id`, `status`, `dedup` döner (motorun iç alanları geçmez).
- YARIŞ: `upsert` `update: {}` ile Prisma 5.22'de SELECT + INSERT'e düşer (atomik değil; kod incelemesi sorgu günlüğüyle ölçtü). Aynı dosyayı aynı anda yükleyen ikinci istek P2002 alır → kazananın satırı okunur, aynı firmaysa yükleme geçer, başka firmaysa kimlik verilmez.
- `/debug/*` artık iç jeton ister (26.09): jetonsuzken iç ağdaki her konteyner bilinmeyen kimlikte `all_keys` ile başka firmaların kimliklerini, kayıtta hash/kapsam/katmanları okuyabiliyordu. Kullanan araç yoktu (ölçüldü); işletmeci `X-Internal-Token` ile çağırır. `/health` jetonsuz.
- KARAR (Emre 26.09): kaydı OLMAYAN file_id'ye geçiş izni (28.08 "bilinçli açıklık", tablo boş başladığı için) KAPANDI → 403, yanıtı başka firmanın dosyasıyla AYNI (var/yok ayırt edilemez). Ölçüldü (canlı 26.09): motordaki 2 kaydın 2'si kayıtlı, DwgDosya'da biçimsiz kimlik 0 → kimse erişim kaybetmedi. Sonuç: sahiplik yazılamazsa yükleme HATA verir (yutulmaz; yoksa dosya sahibine de kapanırdı), kullanıcı yeniden yükler, motor aynı firmada aynı kimliği döner.
- file_id biçimi `^[0-9a-f]{12}$` (motor `uuid4().hex[:12]`): Nest `dogrula` biçimsizi 400 verir, DB'ye ve motora gitmez; motorda yol kuran her yardımcı (`_cache_path` vb.) doğrular → dizin dışına yol kurulamaz, biçimsiz kimlik 400. Test fikstürü de motor biçiminde kimlik kullanır.
- Günlük notu: motor `logging.info` satırlarını yapılandırmıyor — "Upload dedup" izi canlı günlükte GÖRÜNMEZ; tekilleştirme olayları ancak Nest tarafında (ERROR satırları) izlenir.
...
- Kapılar: `test:dwg-kiraci-dedup` (R/L/U/F/G/Y, regresyonda) · `test:guvenlik2` G2-e/G2-g · `python/tests/test_dedup_kapsam.py` (pytest, CI'da YOK — elle koşulur).
