...
| ✅ | `.github/workflows/regression.yml:1-37` (+ `keep-alive.yml:1-52`) | **CI iki bağımsız iş.** Tetik: master'a push VE master'a PR. İkisi de ubuntu-latest + node 20 + npm önbelleği. Sunucu işi: checkout → `npm ci --legacy-peer-deps` → `npx prisma generate` → `npx tsc --noEmit` → **`npm run test:regression`**. ⚠ Haritanın eski notu ("fixture'lar repoda yok, CI ölçemiyor") **ARTIK GEÇERSİZ** — PK3 turunda 20 fixture repoya alındı; CI fiilen ölçüyor. | HARİTA TAM turu 03.08 |
...
| `backend/test/guvenlik-uclari-test.ts` | Uç güvenliği sözleşmesi (K1/K2/K3), DB'siz. K1: `matching.controller.ts`'in `backfill-tags` + `generate-tags` uçları yorumlarında "Admin:" dediği hâlde RolesGuard/@Roles taşımıyor — RolesGuard'ın FİİLEN okuduğu `getAllAndOverride([metot, sınıf])` ile ölçülür; ★KALKAN assertleri aynı sınıftaki ALTI normal-kullanıcı ucunun admin İSTEMEDİĞİNİ korur (sınıf düzeyine @Roles konursa kırılır). K2: gerçek `TierGuard` + sahte ExecutionContext ile ölçer — `LaborController` @RequireTier'i SINIF düzeyinde tuttuğu ve guard yalnız `getHandler()` okuduğu için CORE kullanıcı BEŞ uçtan da geçiyor. K3: sahte Prisma casusuyla `materials.service.ts` `deletePrice`'ın varlık kontrolü yapmadığını ve `priceListId` kapsamı taşımadığını (yazma/silme asimetrisi) kanıtlar; uç zaten admin-korumalı olduğu için o kısım ★KALKAN. O1-O5 ölçüt kontrol vakaları her iddianın yanında sağlam bir örneği aynı ölçüme sokar. **ARTIK YEŞİL** (35/0) — üç kusur da aynı gün düzeltildi; kırmızıya dönerse bu bir REGRESYONDUR |
