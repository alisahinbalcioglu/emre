...
| `frontend/ortak/kabuk/components/landing/NasilCalisir.tsx` | Tanitim sayfasindaki "Nasil Calisir" bolumu: 3 adim, 11 gercek ekran goruntusu (sekmeli), donen tanitim videosu, 4 ozellik karti. Disaridan teslim edilen `BOLUM.html`'in React karsiligi. Teslimdeki vanilya JS KULLANILMADI: `document`'e ekledigi `keydown` dinleyicisini hic kaldirmiyordu (SPA'da birikir) ve `img.src`'yi elle degistirmesi React ile catisiyordu — sekme/buyutme state'e tasindi, dinleyiciler temizleniyor. `.nc-buyut` KOSULLU RENDER EDILMEZ; CSS `display:none` + `.acik{display:flex}` sozlesmesine dayanir. Video `muted` kalmali, yoksa tarayici otomatik oynatmayi engeller |
