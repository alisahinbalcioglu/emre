...
| ✅ | `frontend/components/excel-grid/useFillHandle.tsx:88-270` → `ExcelGrid.tsx:1750-1913` → `fill-down.ts:141-275` | **Sürükle-doldur ÜÇ katman:** (1) JEST — hücrenin alt 10px'inden sürükleme/çift tık, iş kuralı bilmez, yalnız hedef satırları bildirir; (2) YÖNLENDİRİCİ `handleFillComplete` — alana göre 6 dal; (3) MANTIK `fill-down.ts` **yalnız `_marka`/`_firma`** dallarını koşar (her satır kendi çapıyla motora yeniden sorulur, hiçbir satır sessiz kalmaz). Kar%/iskonto dalları hâlâ `ExcelGrid.tsx` içinde satır içi (`:1857-1891`). AG-Grid'in kendi fill özelliği KULLANILMIYOR (grep: 0). | HARİTA TAM turu 03.08 — üç dosya tam okundu |
