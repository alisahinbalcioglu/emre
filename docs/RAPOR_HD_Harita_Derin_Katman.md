...
2. ~~**`saveMaterialsFromSheets` ProductIndex YAZMIYOR**~~ **KAPANDI (P2-2, 03.08).** Dual-write eklendi (`rowKey` çakışma soneki `saveBulkMaterials` ile birebir). Ölçüm bir düzeltme daha getirdi: raporun *"eşleştirme motoru onu göremiyor"* ifadesi **yanlıştı** — motor görüyordu, ama düşük kaliteli manuel çıkarımla. Daha görünür sonuç `brands.service`'in havuz görünümünün legacy birleşik-ad dalına düşmesiydi (Bağlantı/Boy/Kod/Not kolonları kaybolur). **Yan risk kapatıldı:** indeks 0→>0 olunca `importFromIndex` idempotentliği `productIndexId`ye geçtiği için legacy aktarılmış listelerde MÜKERRER kütüphane satırı doğuyordu; artık legacy satırı olan liste indeks yoluna geçmiyor (`library.service.ts`), kapı `test:p2-2` I3c.
3. ~~**Ölü beşinci çevrim noktası:**~~ **KAPANDI (P2-3, 03.08).** Ölçüm iddianın ikinci yarısını çürüttü: `saveBulkMaterials` ölü değil, **beş yerden çağrılıyor** — ölü olan yalnız `exchangeRate` *parametresiydi*, ve **ikizi** `labor-firms.service.ts:706`'daydı (rapor bunu kaçırmıştı). İkisi de kaldırıldı; hiçbir çağıran değer geçmediği için canlı davranış değişmedi.
...
5. **Sürükle-doldur'un "izole modül" vaadi yarım:** ~~kar% ve iskonto~~ → **kar% KAPANDI (P2-1a, 03.08)** ve kozmetik değilmiş: satır içi hali `hesaplaSatisBirimFiyat`/`hesaplaSatirToplam`'ı **hiç çağırmıyordu** (ham çarpım + `toFixed(2)`) → net 3.019,2 · kar %10 · miktar 3'te aynı sütuna **9.963,36** yazıyordu, mühürlü yol ise **9.963,6**. "Çift kar" ile aynı sınıf hata. Artık `fill-down.karYayilimi`, kapı `kar-yayilimi.test.ts`. **İskonto dalı hâlâ satır içi** (`applyDiscountBulk`).
