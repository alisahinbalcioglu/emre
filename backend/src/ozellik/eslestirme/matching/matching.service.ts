...
export class MatchingService {
  ...
  async remember(userId: string, brandId: string, materialName: string, secilenAd: string) {
    ...
    // Yazilacak satirlarin FIRMA sahipligi.
    // ⚠ 29.08 BIRLESME NOTU: bu satirin yorumu "okuma hala kisi bazli" diyordu;
    // ARTIK DOGRU DEGIL. Aday havuzu, capraz-marka alternatifleri ve indeks
    // rozeti G8'de FIRMAYA gecti (`where: { firmaId: k.firmaId }`).
    // Kisi bazli KALAN tek sey HAFIZANIN KENDISI: `EslesmeHafizasi` tekilligi
    // `@@unique([userId, imza])` ve eslesme tercihi kisiseldir. Bu yuzden
    // `firmaIdBul` hala gerekli — `remember` imzasi userId tasiyor ve satirin
    // FIRMA sahipligi buradan turetiliyor.
