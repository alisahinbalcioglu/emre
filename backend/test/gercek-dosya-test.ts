...
async function run() {
  ...
  // kolonlardaki shared zincirler once degere dondurulur.
  {
    ...
    try { sonuc = await qsvc.exportPricedXlsx({ userId: 'u1', firmaId: 'f-u1', teklifKapsami: 'firma' } as any, 'q1'); } catch (e: any) { hata = e?.message ?? 'hata'; }
  ...
  // (kolon farketmeksizin) once degere dondurulur.
  {
    ...
    const qsvc = new QuotesService(prisma, fx, sahteCeviri as any);
    let hata = ''; let sonuc: any = null;
    try { sonuc = await qsvc.exportPricedXlsx({ userId: 'u1', firmaId: 'f-u1', teklifKapsami: 'firma' } as any, 'q9'); } catch (e: any) { hata = e?.message ?? 'hata'; }
  ...
  // → fiyat + toplam (etkinMiktar kurali) → IKI export → cikti dogrulama.
  {
    ...
    const priced = await qsvc.exportPricedXlsx({ userId: 'u1', firmaId: 'f-u1', teklifKapsami: 'firma' } as any, 'q1');
