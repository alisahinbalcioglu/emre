 ...
 *   1. grid            — AutoCAD-vari arka plan
 *   2. backgroundLines — geometry.lines layer bazli batch
 *   3. calculatedEdges — edge_segments cap bazli batch + hit-test (metraj)
 *   4. arcs            — block icinden gelen yariciap yaylar
 *   5. circles         — sprinkler/sembol cemberleri
 *   6. inserts         — ekipman noktalari (hit-target)
 *   7. texts           — TEXT/MTEXT (zoom>=0.3 LOD)
 */
...
export default function DxfPixiViewer({
  ...
  useEffect(() => {
    ...
    // 5xx (cold start): 3s/8s/20s — Render free tier ~50sn cold start covered.
    //   Toplam bekleme ~31sn + ilk + son request = ~32-35sn cold-start tolerans.
    // 429 (Cloudflare burst): 10s/20s/30s — cooldown'a saygi.
    const RETRY_DELAYS_5XX_MS = [3000, 8000, 20000];
    ...
    (async () => {
      ...
      for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        try {
          ...
          // Delay: 429 ozel (Retry-After veya artan), 5xx artan delay
          let delay: number;
          if (status === 429) {
            ...
            delay = RETRY_DELAYS_5XX_MS[attempt] ?? 20000;
            // Cold start senaryosu — kullanici beklerken durumu bilsin
            if (!cancelled) setError(`Sunucu uyandiriliyor, ~${Math.round(delay / 1000)}sn sonra tekrar denenecek...`);
          }
